...
def pnlpic(pnl: int, symbol: str, invest: int, worth: int, userid: int): 
    ...
    x_middle_left_1 = 60  # Adjust to leave space for the arrow
    x_middle_left_2 = 10 
    ...
    # Draw the arrow 
    arrow_y_offset = y_middle_left_1 + text_height_middle_left_1 // 2 - 50  # Center the arrow vertically
    arrow_x_offset = 10  # Position the arrow to the left of the percentage text

    if pnl >= 0:
        # Draw upward arrow
        draw.polygon([(arrow_x_offset, arrow_y_offset + 100), 
                      (arrow_x_offset + 50, arrow_y_offset), 
                      (arrow_x_offset + 100, arrow_y_offset + 100)], fill=color)
    else:
        # Draw downward arrow
        draw.polygon([(arrow_x_offset, arrow_y_offset), 
                      (arrow_x_offset + 50, arrow_y_offset + 100), 
                      (arrow_x_offset + 100, arrow_y_offset)], fill=color)
 
    image.save(f'media/output{userid}.png')

# Example usage:
pnlpic(-1050, "nigger", 8, 5, 12345)